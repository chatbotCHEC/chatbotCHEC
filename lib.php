...
class chatBotAPI {
    ...
    public function getIndisponibilidad($niu){
        ...
        if(count($circuito) > 0 && ($circuito->ESTADO =="ABIERTO" || $circuito->ESTADO =="APERTURA")){
            $msg.="\n *Para la cuenta número \$number, hemos encontrado las siguientes indisponibilidades: \n - Hay una falla en el circuito reportada el ".$circuito->FECHA." a las ".$circuito->HORA.". Estamos trabajando para reestablecer el servicio";
        }else {
            $msg.="\n *Para la cuenta número \$number no tengo reportada ninguna indisponibilidad";
        }
    ...
    public function getSuspensionesProgramadas($niu){
        ...
        $msg = "";
        if(count($prog)>0){
            $msg.="\n *Para la cuenta con el número \$number, hemos encontrado las siguientes suspensiones programadas: ";
            foreach ($prog as $p) {
            ...
            $msg.="\n *Para la cuenta con el número \$number no tengo reportada ninguna suspensión programada";
        }
